...
- Together (watch parties, docs/TOGETHER.md) publishes an authoritative *timeline* (media position pinned to a relay timestamp), not events. Clients steer onto it continuously: rate nudges under ±5% for small drift, seeks only as a confirmed last resort — a seek re-primes the audio device and is exactly the "audio early/late" bug. Room rules and sync maths are pure in `src/shared/together/`; the socket lives in main. NAT is handled by mesh VPN (ZeroTier/Tailscale) detection + one-click winget install, never by bundling a VPN.
